...
function LogMethodCalls(target: any, propertyName: string, propertyDescriptor: PropertyDescriptor): PropertyDescriptor {
  ...
  propertyDescriptor.value = function (...args: any[]) {
    // Форматуємо аргументи як рядок
    const formattedArgs = args.join(', ');

    // Лог повідомлення з відформатованими аргументами
    console.log(`Calling "${propertyName}" with arguments: ${formattedArgs}`);
...
namespace UserProfile {
  ...
  function generateId(): string {
    return Math.random().toString(36).substr(2, 9); // Генерує випадковий рядок довжиною 9 символів
  }
}     
